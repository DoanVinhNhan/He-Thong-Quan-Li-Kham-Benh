...
# --- Cấu trúc Radix Tree (Trie) ---
class RadixTreeNode:
    def __init__(self):
        self.children = HashTable(initial_table_size=10)
...
class RadixTree:
    ...
    def insert(self, key_str, value):
        """
        Chèn một chuỗi khóa (key_str) và giá trị liên quan (value) vào cây.
        Giá trị thường là patient_id.
        """
    ...
    def search(self, key_str):
        """
        Tìm kiếm một chuỗi khóa (key_str) chính xác trong cây.
        Trả về giá trị (patient_id) nếu tìm thấy, ngược lại None.
        """
        if not isinstance(key_str, str):
            return None
    ...
    def delete(self, key_str):
        """
        Xóa một khóa khỏi cây. Đây là một phiên bản đơn giản hóa.
        Nó chỉ bỏ đánh dấu is_end_of_key và xóa giá trị.        """
        if not isinstance(key_str, str):
            return False

        node_stack_for_cleanup = List()
        ...
        current_node.is_end_of_key = False
        ...
        for i in range(len(node_stack_for_cleanup) -1, -1, -1):
            ...
            if len(child_node.children) == 0 and not child_node.is_end_of_key:
