...
# from custom_structures import PriorityQueue # Import này không còn cần thiết trực tiếp ở đây
...
class AppGUI(ctk.CTk):
    ...
    def _setup_dang_ky_tab(self):
        # Sử dụng CTkFrame thường cho frame chính của tab này
        ...
        # --- 1. Phần Đăng Ký Khám (đặt lên đầu) ---
        ...
        ctk.CTkLabel(actual_dk_kham_form_frame, text="Mã BN để đăng ký khám:").grid(row=0, column=0, padx=5, pady=5, sticky="w") # giảm pady
        ...
        # --- 2. Phần Quản Lý Hồ Sơ Bệnh Nhân (đặt ở dưới) ---
        ...
        for i, (label_text, key) in enumerate(fields):
            ...
            if key in ["địa_chỉ", "tiền_sử_bệnh_án", "dị_ứng_thuốc"]: 
                widget = ctk.CTkTextbox(actual_ho_so_form_frame, height=45, width=330, border_width=2) 
            else:
                widget = ctk.CTkEntry(actual_ho_so_form_frame, width=330) 
            widget.grid(row=i+1, column=1, columnspan=2, padx=5, pady=field_pady, sticky="ew")
            self.entries_dk[key] = widget
    ...
    def _load_patient_for_edit(self):
        ...
        if bn:
            ...
            self.entries_dk["cccd"].insert(0, bn.cccd or "")
            ...
            messagebox.showinfo("Thông báo", f"Đã tải thông tin BN: {ma_bn} - {bn.ho_ten} vào form Hồ sơ và Đăng ký khám.")
        else:
    ...
    def _tao_moi_ho_so(self): 
        ...
        if not all(data.get(key,"") for key in ["họ_tên", "ngày_sinh", "giới_tính", "sđt", "cccd"]):
        ...
        except ValueError: messagebox.showerror("Lỗi", f"Định dạng Ngày sinh không hợp lệ (YYYY-MM-DD)."); return
    ...
    def _cap_nhat_ho_so(self): 
        ma_bn = self.entry_ma_bn_dk.get().strip()
        if not ma_bn: messagebox.showerror("Lỗi", "Vui lòng nhập Mã BN của hồ sơ cần cập nhật (hoặc Tải BN)."); return
        ...
        if not update_data_raw.get("cccd",""): 
            messagebox.showerror("Lỗi", "Số CCCD không được để trống khi cập nhật.")
            return
        if update_data_raw.get("ngày_sinh") and update_data_raw["ngày_sinh"].strip() != "": 
        ...
        result = self.he_thong.cap_nhat_thong_tin_benh_nhan(ma_bn, **update_data)
        if result: 
            messagebox.showinfo("Thành công", f"Đã cập nhật hồ sơ cho BN: {ma_bn}")
            self._refresh_all_patient_lists()
        elif result is False and self.he_thong.tim_benh_nhan_theo_ma(ma_bn) is not None : 
             messagebox.showinfo("Thông báo", f"Không có thông tin nào được thay đổi cho BN: {ma_bn}")
    ...
    def _xoa_ho_so(self): 
        ...
        if messagebox.askyesno("Xác nhận xóa", f"Bạn có chắc chắn muốn xóa hồ sơ BN: {ma_bn}? {warning_message}"):
            if self.he_thong.xoa_ho_so_benh_nhan(ma_bn):
                ...
                self._refresh_all_patient_lists()
            else: messagebox.showerror("Lỗi", f"Không thể xóa hồ sơ BN: {ma_bn}. Chi tiết xem console.")
    ...
    def _dang_ky_kham(self): 
        ...
        if not ma_bn_kham_input or ma_bn_kham_input == self.entry_ma_bn_kham.cget("placeholder_text"): 
            messagebox.showerror("Lỗi", "Vui lòng nhập Mã BN (hoặc tải BN từ form hồ sơ) để đăng ký khám."); return
        priority_str = self.combo_priority_dk.get()
        if not priority_str: messagebox.showerror("Lỗi", "Vui lòng chọn mức độ ưu tiên."); return
        if self.he_thong.dang_ky_kham_benh(ma_bn_kham_input, priority_str):
            messagebox.showinfo("Thành công", f"BN {ma_bn_kham_input} đã được thêm vào hàng đợi với ưu tiên '{priority_str}'.")
            self._refresh_hang_doi_list()
            self.entry_ma_bn_kham.delete(0,"end") 
    ...
    def _setup_hang_doi_tab(self):
        ...
        controls_frame.pack(pady=10, padx=10) # Căn giữa frame này
        ...
        change_prio_outer_frame = ctk.CTkFrame(frame, fg_color="transparent")
        change_prio_outer_frame.pack(pady=10, fill="x", padx=10)
        ctk.CTkLabel(change_prio_outer_frame, text="Thay đổi Ưu tiên BN trong Hàng đợi:", font=("Arial", 12, "bold")).pack(anchor="w", pady=(0,5))
        
        change_prio_frame = ctk.CTkFrame(change_prio_outer_frame)
        change_prio_frame.pack(fill="x")
        ctk.CTkLabel(change_prio_frame, text="Mã BN:").grid(row=0, column=0, padx=(5,0), pady=5, sticky="e")
        self.entry_change_prio_ma_bn = ctk.CTkEntry(change_prio_frame, width=120)
        self.entry_change_prio_ma_bn.grid(row=0, column=1, padx=5, pady=5, sticky="w")
        ctk.CTkLabel(change_prio_frame, text="Ưu tiên mới:").grid(row=0, column=2, padx=(10,0), pady=5, sticky="e")
        self.combo_change_prio_new_level = ctk.CTkComboBox(change_prio_frame, values=PRIORITY_LEVELS, width=160)
        self.combo_change_prio_new_level.grid(row=0, column=3, padx=5, pady=5, sticky="w")
        ...
        self.listbox_hang_doi.column("STT", width=40, anchor="center")
        self.listbox_hang_doi.column("MaBN", width=80, anchor="center")
        self.listbox_hang_doi.column("HoTen", width=220)
        self.listbox_hang_doi.column("UuTien", width=150, anchor="center")
        self.listbox_hang_doi.column("TGDangKy", width=100, anchor="center")
        self.listbox_hang_doi.column("SoLanVang", width=50, anchor="center")
        scrollbar = ttk.Scrollbar(tree_frame, orient="vertical", command=self.listbox_hang_doi.yview)
        self.listbox_hang_doi.configure(yscrollcommand=scrollbar.set)
        scrollbar.pack(side="right", fill="y")
        self.listbox_hang_doi.pack(expand=True, fill="both")
        
        # Đã xóa action_frame_bottom và các nút trong đó
        ...
        if display_strings_from_logic and display_strings_from_logic[0] == "Hàng đợi rỗng.": pass
            ...
            for display_str in display_strings_from_logic:
                try:
                    ...
                    stt_part = parts[0].split('.')[0].strip() 
                    ma_bn_part = parts[0].split('ID:')[1].strip() 
                    ho_ten_part = parts[1].split('Tên:')[1].strip()
                    uu_tien_part = parts[2].split('Ưu tiên:')[1].strip()
                    tg_dk_part = parts[3].split('TGĐK:')[1].strip()
                    so_lan_vang_part = parts[4].split('Vắng:')[1].strip()
                    
                    self.listbox_hang_doi.insert("", "end", values=(
                        stt_part, ma_bn_part, ho_ten_part, uu_tien_part, tg_dk_part, so_lan_vang_part
                    ))
    ...
    def _goi_kham(self): 
        if self.benh_nhan_dang_kham:
            messagebox.showwarning("Lưu ý", f"BN {self.benh_nhan_dang_kham.profile.ho_ten} đang trong quá trình khám. Hoàn thành khám trước.")
            return
        ...
        if benh_nhan_kham_obj: 
            ...
            messagebox.showinfo("Gọi khám", f"Đã gọi BN: {bn_info.ho_ten} (ID: {bn_info.ma_bn})")
            self._refresh_hang_doi_list()
        else:
            self.label_dang_kham.configure(text="Đang khám: Hàng đợi rỗng"); self.benh_nhan_dang_kham = None
            messagebox.showinfo("Thông báo", "Hàng đợi khám rỗng.")
    ...
    def _hoan_thanh_kham(self): 
        ...
        if ghi_chu is None: ghi_chu = ""
        self.he_thong.hoan_thanh_kham(ma_bn_kham, ket_qua, ghi_chu)
        messagebox.showinfo("Hoàn thành", f"BN {ma_bn_kham} ({ho_ten_bn_kham}) đã khám xong.")
    ...
    def _bn_vang_mat(self): 
        ...
        if messagebox.askyesno("Xác nhận vắng mặt", 
            ...
            self.label_dang_kham.configure(text="Đang khám: Chưa có"); self.benh_nhan_dang_kham = None 
            if bi_loai_bo: messagebox.showinfo("Thông báo", f"BN {patient_called_obj.patientID} đã bị loại do vắng mặt đủ số lần.")
            else: messagebox.showinfo("Thông báo", f"BN {patient_called_obj.patientID} đã được ghi nhận vắng mặt và xử lý.")
            self._refresh_hang_doi_list() 
    ...
    def _bn_roi_di(self): 
        ...
        if ma_bn_roi and ma_bn_roi.strip():
            if self.he_thong.benh_nhan_roi_di_khi_dang_cho(ma_bn_roi.strip()):
                ...
                self._refresh_hang_doi_list()
            else: messagebox.showerror("Lỗi", f"Không thể xóa BN {ma_bn_roi.strip()} khỏi hàng đợi (có thể không tìm thấy).")
        elif ma_bn_roi is not None: messagebox.showwarning("Cảnh báo", "Mã BN không được để trống.")

    def _update_priority_long_wait(self): 
        # Nút này đã bị xóa theo yêu cầu, nhưng logic vẫn giữ lại nếu bạn muốn dùng ở đâu đó
        ...
        if thoi_gian_cho_str:
            try:
                ...
                if thoi_gian_cho_giay <= 0: messagebox.showerror("Lỗi", "Thời gian chờ phải là số dương."); return
                self.he_thong.cap_nhat_uu_tien_cho_benh_nhan_cho_lau(thoi_gian_cho_giay)
                self._refresh_hang_doi_list()
            except ValueError: messagebox.showerror("Lỗi", "Thời gian chờ phải là một số nguyên.")
    ...
    def _thay_doi_uu_tien_trong_hang_doi(self): 
        ...
        if not ma_bn: messagebox.showerror("Lỗi", "Vui lòng nhập Mã BN cần thay đổi ưu tiên."); return
        if not new_priority_str: messagebox.showerror("Lỗi", "Vui lòng chọn Mức ưu tiên mới."); return
        if messagebox.askyesno("Xác nhận thay đổi", f"Thay đổi ưu tiên của BN {ma_bn} thành '{new_priority_str}'?"):
            success = self.he_thong.thay_doi_uu_tien_bn_trong_hang_doi(ma_bn, new_priority_str)
            if success:
                messagebox.showinfo("Thành công", f"Đã cập nhật ưu tiên cho BN {ma_bn}.")
                self._refresh_hang_doi_list() 
                self.entry_change_prio_ma_bn.delete(0, "end") 
            else:
                messagebox.showerror("Lỗi", f"Không thể thay đổi ưu tiên cho BN {ma_bn}. Có thể BN không trong hàng đợi hoặc mức ưu tiên không hợp lệ.")
    ...
    def _tim_kiem_benh_nhan(self): 
        ...
        else: messagebox.showinfo("Thông báo", "Nhập ít nhất một tiêu chí tìm kiếm, hoặc nhấn 'Hiển thị Tất cả BN'."); self._display_search_results([], title="Vui lòng nhập tiêu chí."); return
    ...
    def _setup_da_kham_tab(self): 
        ...
        # self._refresh_da_kham_list() # Đã gọi ở __init__
