# models.py
import datetime
from custom_structures import LinkedList, List # Sử dụng tên lớp đã đổi
...
class Doctor: 
    def __init__(self, doctor_id, doctor_name, specialty, clinic_id_list_str=""): 
        self.doctor_id = doctor_id; self.doctor_name = doctor_name; self.specialty = specialty
        ...
        if clinic_id_list_str:
            ...
            for pk_id in clinic_ids_py:
                if pk_id.strip(): self.clinic_id_list.append(pk_id.strip())
    def to_csv_row(self):
        clinic_str_list_py = []; [clinic_str_list_py.append(self.clinic_id_list.get(i)) for i in range(len(self.clinic_id_list))]
        return {"ma_bac_si": self.doctor_id, "ho_ten_bac_si": self.doctor_name, "chuyen_khoa": self.specialty,
                "danh_sach_ma_phong_kham": LIST_ID_SEPARATOR.join(clinic_str_list_py)}
    @classmethod
    def from_csv_row(cls, row_data): 
        return cls(doctor_id=row_data.get("ma_bac_si", ""), doctor_name=row_data.get("ho_ten_bac_si", ""),
                   specialty=row_data.get("chuyen_khoa", ""), clinic_id_list_str=row_data.get("danh_sach_ma_phong_kham", ""))
    def __str__(self): return f"BS: {self.doctor_id} - {self.doctor_name} ({self.specialty})"

class Clinic: 
    def __init__(self, clinic_id, clinic_name, clinic_specialty_val, doctor_id_list_str=""): 
        self.clinic_id = clinic_id; self.clinic_name = clinic_name; self.clinic_specialty = clinic_specialty_val
        self.doctor_id_list = List()
        if doctor_id_list_str:
            bs_ids_py = doctor_id_list_str.split(LIST_ID_SEPARATOR)
            for bs_id in bs_ids_py:
                if bs_id.strip(): self.doctor_id_list.append(bs_id.strip())
    def to_csv_row(self):
        doctor_str_list_py = []; [doctor_str_list_py.append(self.doctor_id_list.get(i)) for i in range(len(self.doctor_id_list))]
        return {"ma_phong_kham": self.clinic_id, "ten_phong_kham": self.clinic_name,
                "chuyen_khoa_pk": self.clinic_specialty, "danh_sach_ma_bac_si": LIST_ID_SEPARATOR.join(doctor_str_list_py)}
    @classmethod
    def from_csv_row(cls, row_data): 
        return cls(clinic_id=row_data.get("ma_phong_kham", ""), clinic_name=row_data.get("ten_phong_kham", ""),
                   clinic_specialty_val=row_data.get("chuyen_khoa_pk", ""), doctor_id_list_str=row_data.get("danh_sach_ma_bac_si", ""))
    def __str__(self): return f"PK: {self.clinic_id} - {self.clinic_name} ({self.clinic_specialty})"
...
class Patient: 
    def __init__(self, patient_id, full_name, date_of_birth_val, gender, address, phone_number, national_id, 
        ...
        self.patient_id = patient_id; self.full_name = full_name
        ...
        self.gender = gender; self.address = address; self.phone_number = phone_number; self.national_id = national_id 
        self.health_insurance_id = health_insurance_id; self.medical_history_summary = medical_history_summary_val; self.drug_allergies = drug_allergies_val
        ...
        if examination_history_str: self._deserialize_examination_history(examination_history_str)
    ...
    @classmethod
    def from_csv_row(cls, row_data): 
        return cls(patient_id=row_data.get("ma_bn", ""), full_name=row_data.get("ho_ten", ""), date_of_birth_val=row_data.get("ngay_sinh", ""), 
                   gender=row_data.get("gioi_tinh", ""), address=row_data.get("dia_chi", ""), phone_number=row_data.get("sdt", ""),
                   national_id=row_data.get("cccd", "N/A_CSV_ERROR"), health_insurance_id=row_data.get("bhyt", ""), 
                   medical_history_summary_val=row_data.get("tien_su_benh_an", ""), drug_allergies_val=row_data.get("di_ung_thuoc", ""),
...
class PatientInQueue: 
    ...
    def __init__(self, patient_profile_obj, priority_str_val, registration_timestamp=None): 
        ...
        if not self.create_and_set_priority(priority_str_val): raise ValueError(f"Mức ưu tiên không hợp lệ: {priority_str_val}")
    ...
    def create_and_set_priority(self, priority_str_val): 
        if priority_str_val in self.PRIORITY_MAP: self.priority = self.PRIORITY_MAP[priority_str_val]; return True
        return False
    def increment_absent_count(self): self.absent_count += 1 
    def should_leave_queue(self): return self.absent_count >= 3 
    def get_priority_display_name(self): return self.PRIORITY_DISPLAY_MAP.get(self.priority, "Không xác định") 
    def __str__(self): return (f"ID:{self.patient_id},Tên:{self.patient_profile.full_name},Ưu tiên:{self.get_priority_display_name()}({self.priority}),TGĐK:{self.registration_time.strftime('%H:%M:%S')},Vắng:{self.absent_count}")
    def __gt__(self, other_patient_in_queue):
        if self.priority != other_patient_in_queue.priority: return self.priority > other_patient_in_queue.priority
