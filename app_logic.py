...
# Hàm để lấy đường dẫn tài nguyên chính xác
...
class MedicalSystemLogic:
    ...
    def _load_data_from_csv(self, csv_filepath, model_class_ref, target_hash_table_obj, id_update_callback, key_attribute_name, id_prefix):
        ...
        if csv_fieldnames_custom_array.is_empty():
            print(f"Không có fieldnames được định nghĩa cho {model_class_ref.__name__}. Không tải."); return

        csv_fieldnames_py_list = self._convert_custom_list_to_py_list(csv_fieldnames_custom_array)
        required_fields_map_dict = {
        ...
        try:
            with open(csv_filepath, mode='r', encoding='utf-8', newline='') as csvfile:
                ...
                if not csv_reader.fieldnames or not all(f_name in csv_reader.fieldnames for f_name in required_field_list):
                     print(f"Lỗi: Header của tệp {csv_filepath} không khớp hoặc thiếu các cột quan trọng. Không tải dữ liệu."); return

                for row_idx, data_row in enumerate(csv_reader, 1):
                    try:
                        if not all(key in data_row and data_row[key] for key in required_field_list):
                            # print(f"Cảnh báo: Dòng {row_idx} trong {csv_filepath} thiếu dữ liệu ở cột bắt buộc. Bỏ qua dòng này.")
                            continue # Bỏ qua dòng nếu thiếu dữ liệu ở cột bắt buộc
                        ...
                        if model_class_ref == Patient and (not item_instance.national_id or item_instance.national_id in ["N/A_DEFAULT", "N/A_CSV_ERROR"]):
                            # print(f"Cảnh báo: Dòng {row_idx} trong {csv_filepath} có CCCD không hợp lệ. Bỏ qua bệnh nhân này.")
                            continue # Bỏ qua bệnh nhân nếu CCCD không hợp lệ
                        ...
                        # Điền dữ liệu vào Radix Trees cho bệnh nhân
                        ...
                        if item_unique_id.startswith(id_prefix):
                            ...
                            except ValueError: continue # Bỏ qua nếu phần số của ID không hợp lệ
                            if id_numeric_part > max_id_val: max_id_val = id_numeric_part
                    except Exception as row_exception:
                        print(f"Lỗi khi xử lý dòng {row_idx} trong tệp {csv_filepath}: {row_exception}")
            id_update_callback(max_id_val + 1)
            print(f"Đã tải thành công {loaded_items_count} mục từ {csv_filepath}. Next ID cho {model_class_ref.__name__}: {getattr(self, f'next_{model_class_ref.__name__.lower()}_id_counter', max_id_val + 1)}")
        except FileNotFoundError:
             print(f"LỖI NGHIÊM TRỌNG: Tệp {csv_filepath} không được tìm thấy mặc dù đã kiểm tra trước đó. Điều này không nên xảy ra.")
        except Exception as load_exception:
            print(f"Lỗi nghiêm trọng khi tải tệp {csv_filepath}: {load_exception}")
    ...
    def _get_save_path(self, csv_filename_const):
        ...
        if getattr(sys, 'frozen', False):
            ...
            # Chạy từ script Python (chế độ dev)
    ...
    def _save_data_to_csv(self, csv_filename_const, model_class_ref, source_hash_table_obj):
        ...
        print(f"Đang cố gắng lưu vào: {actual_csv_filepath}")
        ...
        saved_items_count = 0
        try:
            # Tạo thư mục nếu chưa tồn tại (quan trọng khi lưu vào thư mục con của .exe)
            os.makedirs(os.path.dirname(actual_csv_filepath), exist_ok=True)
            ...
            with open(actual_csv_filepath, mode='w', encoding='utf-8', newline='') as csvfile:
                ...
                for i in range(len(all_items_custom_array)):
                    ...
                    if isinstance(current_item, model_class_ref):
                        ...
                        saved_items_count +=1
            print(f"Đã lưu thành công {saved_items_count} mục vào {actual_csv_filepath}")
        except IOError as e:
            print(f"Lỗi IO khi lưu tệp {actual_csv_filepath}: {e}. Kiểm tra quyền ghi.")
        except Exception as save_exception:
            print(f"Lỗi không xác định khi lưu tệp {actual_csv_filepath}: {save_exception}")
    ...
    def create_patient_record(self, full_name_val, dob_str, gender_val, address_val, phone_val, national_id_val, health_insurance_id_val="", medical_history_val="", drug_allergies_val=""):
        ...
        if self.national_id_radix_tree.search(cleaned_national_id):
             return None, f"Số CCCD '{cleaned_national_id}' đã tồn tại trong hệ thống.", "ERROR"

        cleaned_phone = phone_val.strip()
        if self.phone_radix_tree.search(cleaned_phone):
            return None, f"Số điện thoại '{cleaned_phone}' đã tồn tại trong hệ thống.", "ERROR"

        new_patient_id = self._generate_patient_id()
        if self.patient_records_table.contains_key(new_patient_id): return None, f"Mã BN {new_patient_id} đã tồn tại (lỗi logic tạo ID).", "ERROR"
        ...
        if cleaned_phone:
            self.phone_radix_tree.insert(cleaned_phone, new_patient_id)
        if cleaned_national_id:
            self.national_id_radix_tree.insert(cleaned_national_id, new_patient_id)
    ...
    def update_patient_info(self, patient_id_val, **update_kwargs):
        ...
        old_phone = patient_obj.phone_number
        old_national_id = patient_obj.national_id
        ...
        if cleaned_new_national_id and cleaned_new_national_id != old_national_id:
            ...
            if existing_patient_id_for_new_nid and existing_patient_id_for_new_nid != patient_id_val:
                return False, f"Số CCCD '{cleaned_new_national_id}' đã được sử dụng bởi BN khác ({existing_patient_id_for_new_nid}).", "ERROR"
        ...
        if cleaned_new_phone and cleaned_new_phone != old_phone:
            ...
            if existing_patient_id_for_new_phone and existing_patient_id_for_new_phone != patient_id_val:
                return False, f"Số điện thoại '{cleaned_new_phone}' đã được sử dụng bởi BN khác ({existing_patient_id_for_new_phone}).", "ERROR"
            ...
            if hasattr(patient_obj, attr_key) and new_attr_value is not None:
                ...
                if attr_key == "date_of_birth" and isinstance(new_attr_value, str):
                    if not new_attr_value.strip(): final_new_value = None # Cho phép xóa ngày sinh
                        ...
                        except ValueError: return False, f"Ngày sinh '{new_attr_value}' không hợp lệ (YYYY-MM-DD).", "ERROR"
                ...
                if current_attr_value != final_new_value:
                    setattr(patient_obj, attr_key, final_new_value); was_updated = True

        if was_updated:
            # Cập nhật Radix Trees nếu SĐT hoặc CCCD thay đổi
    ...
    def delete_patient_record(self, patient_id_val):
        ...
        if self.patient_records_table.delete_item(patient_id_val):
            if patient_to_delete.phone_number and patient_to_delete.phone_number.strip():
                self.phone_radix_tree.delete(patient_to_delete.phone_number.strip())
            if patient_to_delete.national_id and patient_to_delete.national_id.strip():
                self.national_id_radix_tree.delete(patient_to_delete.national_id.strip())
    ...
    def handle_patient_leaving_queue(self, patient_id_leaving, clinic_id_val):
        ...
        if patient_to_remove_instance:
            ...
            clinic_queue.internal_heap.heap_array = List(); # Tạo lại heap_array
    ...
    def advanced_patient_search(self, **search_criteria):
        ...
        if phone_query_exact:
            ...
            if patient_id_found:
                ...
                if patient_obj:
                    result = List(); result.append(patient_obj); return result
            return List()
        ...
        name_query = search_criteria.get("full_name", "").lower().strip(); phone_query_contains = search_criteria.get("phone_number", "").strip()
        dob_query_str = search_criteria.get("date_of_birth", "").strip(); national_id_query_contains = search_criteria.get("national_id", "").strip(); health_ins_query = search_criteria.get("health_insurance_id", "").strip()
        dob_query_date = None
        if dob_query_str:
            try: dob_query_date = datetime.datetime.strptime(dob_query_str, DATE_FORMAT_CSV).date()
            except ValueError: pass # Nếu ngày sinh không hợp lệ, bỏ qua tiêu chí này
    ...
    def delete_doctor(self, doctor_id_val):
        if self.doctor_records_table.delete_item(doctor_id_val):
            ...
            self._save_data_to_csv(CLINICS_CSV_FILENAME, Clinic, self.clinic_records_table) # Lưu lại cả clinic vì DS bác sĩ trong PK đã thay đổi
    ...
    def delete_clinic(self, clinic_id_val):
        ...
        if self.clinic_records_table.delete_item(clinic_id_val):
            ...
            self._save_data_to_csv(CLINICS_CSV_FILENAME, Clinic, self.clinic_records_table)
            self._save_data_to_csv(DOCTORS_CSV_FILENAME, Doctor, self.doctor_records_table) # Lưu lại BS vì DS PK của BS đã thay đổi
    ...
    def assign_doctor_to_clinic(self, doctor_id_val, clinic_id_val):
        ...
        return False, f"BS {doctor_id_val} đã được gán cho PK {clinic_id_val} từ trước.", "INFO" # Sửa thông báo
    ...
    def remove_doctor_from_clinic(self, doctor_id_val, clinic_id_val):
        ...
        for i in range(len(doc_obj.clinic_id_list)):
            ...
            else: was_removed = True # Đảm bảo was_removed được set nếu BS bị xóa khỏi danh sách PK của BS
        doc_obj.clinic_id_list = new_clinic_id_list_for_doc
        if was_removed:
            self._save_data_to_csv(CLINICS_CSV_FILENAME, Clinic, self.clinic_records_table)
            self._save_data_to_csv(DOCTORS_CSV_FILENAME, Doctor, self.doctor_records_table)
            return True, f"Đã xóa BS {doctor_id_val} khỏi PK {clinic_id_val}.", "INFO"
        return False, f"BS {doctor_id_val} không có trong PK {clinic_id_val}.", "INFO" # Sửa thông báo
    ...
    def _collect_all_examination_history(self):
        ...
        for i in range(len(all_patients_custom_array)):
            ...
            if isinstance(patient_obj, Patient):
                for history_item_dict in patient_obj.examination_history: # examination_history là list Python
                    record_copy = dict(history_item_dict) # Tạo bản sao để tránh thay đổi dữ liệu gốc
    ...
    def filter_examination_history(self, from_date_str=None, to_date_str=None, doctor_id_filter=None, clinic_id_filter=None):
        ...
        if from_date_str:
            ...
            except ValueError: return List(), f"Từ ngày '{from_date_str}' không hợp lệ (YYYY-MM-DD).", "ERROR"
        if to_date_str:
            try: to_date_obj = datetime.datetime.strptime(to_date_str, DATE_FORMAT_CSV).date()
            except ValueError: return List(), f"Đến ngày '{to_date_str}' không hợp lệ (YYYY-MM-DD).", "ERROR"
        ...
        if filtered_py_list:
            def get_sort_key_func(item_dict_val):
                ...
                return exam_date_val if isinstance(exam_date_val, datetime.date) else datetime.date.min # Xử lý trường hợp ngày khám không hợp lệ
            filtered_py_list.sort(key=get_sort_key_func, reverse=True) # Sắp xếp giảm dần theo ngày khám

        final_filtered_custom_array = List() # Chuyển lại Custom List để trả về
        for item_dict in filtered_py_list: final_filtered_custom_array.append(item_dict)

        if final_filtered_custom_array.is_empty(): return List(), "Không có Lịch sử khám nào khớp với tiêu chí lọc.", "INFO"
        return final_filtered_custom_array, f"Tìm thấy {len(final_filtered_custom_array)} kết quả Lịch sử khám.", "INFO"
